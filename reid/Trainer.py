...
class Trainer:
    def __init__(self,pretrained=None,last_epoch=-1):
        ...
        self.sgd = paddle.optimizer.SGD(parameters=self.model.parameters(), weight_decay=5e-4, learning_rate=self.scheduler)
    ...
    def train(self):
        if self.train_loader is None:
            train_ds = ReIDDataset('./data/archive/Market-1501-v15.09.15/bounding_box_train', transform=self._ds_transforms)
            self.train_loader = DataLoader(train_ds, batch_size=32, shuffle=True)
        if self.test_loader is None:
            test_ds = ReIDDataset('./data/archive/Market-1501-v15.09.15/bounding_box_test', transform=self._ds_transforms)
